...
在构造函数中传入一个对象，并且在对象中声明各种Vue需要的数据和方法，包括但不限于：
...
全局注册使用的机会比较少，因为一旦全局注册，就意味着即便以后你不再使用这个组件，它依然会随着Vue的加载而加载。

因此，更多的时候，我们会选择采用局部注册。
...
webpack支持全局安装和本地安装，官方推荐是本地安装，我们安装官方的来。
...
module.exports={
    entry:'./src/main.js',  //指定打包的入口文件
    output:{
        // path: 输出的目录，__dirname是相对于webpack.config.js配置文件的根目录
        path : __dirname+'/dist',  
